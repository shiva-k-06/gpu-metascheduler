...
class HPCSchedulingEnv(gym.Env):
    ...
    def step(self, action):
        ...
        selected_load = len(gpu.queue) + (0 if gpu.is_idle() else 1)

        estimated_completion_time = selected_load + (job.runtime / gpu.speed)

        reward -= estimated_completion_time * 0.05
        reward -= imbalance_penalty * 0.4
        reward -= len(gpu.queue) * 0.6
        reward += gpu.speed * 0.3
